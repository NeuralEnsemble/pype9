...
def bootstrap(module_dir):
    """
    Bootstraps the module directory by running various shell commands
    
    @param module_dir[str]: The path to the module directory to be configured
    """
    # The list of shell commands to run
    cmds="""
#!/bin/sh

echo "Bootstrapping {module_dir}"
...
echo "Done.""".format(module_dir=module_dir)
...
def configure(module_name, module_dir):
    """
    Configures the module directory by running various shell commands
    
    @param module_name[str]: The name of the module to be created
    @param module_dir[str]: The path to the module directory to be configured
    """    
    configure_ac="""
AC_PREREQ(2.52)
...
echo""".format(module_lower=module_name.lower(), module_upper=module_name.upper(), 
                                                    module_capitalized=module_name.capitalized())
    # Write configure.ac with module names to file
    with open(os.path.join(module_dir, 'src', 'configure.ac'), 'w') as f:
        f.write(configure_ac)
    old_dir = os.getcwd()
    os.chdir(os.path.join(module_dir, 'build'))
    sp.check_call(os.path.join(module_dir, 'src', 'configure'))
    os.chdir(old_dir)
        
def make(module_dir, module_name):
    """
    Configures the module directory by running various shell commands
    ...
    @param module_dir[str]: The path to the module directory to be configured
    """   
    # The makefile string to be written to file
    makefile="""
libdir= @libdir@/nest

lib_LTLIBRARIES=      {module_name}.la lib{module_name}.la

{module_name}_la_CXXFLAGS= @AM_CXXFLAGS@
{module_name}_la_SOURCES=  {module_name}.cpp      {module_name}.h      \

{module_name}_la_LDFLAGS=  -module

lib{module_name}_la_CXXFLAGS= $({module_name}_la_CXXFLAGS) -DLINKED_MODULE
lib{module_name}_la_SOURCES=  $({module_name}_la_SOURCES)
...
EXTRA_DIST= sli""".format(module_name=module_name)
    # Write configure.ac with module names to file
    with open(os.path.join(module_dir, 'src', 'Makefile.am'), 'w') as f:
        f.write(makefile)
    old_dir = os.getcwd()
    os.chdir(os.path.join(module_dir, 'build'))
    sp.check_call('make')
