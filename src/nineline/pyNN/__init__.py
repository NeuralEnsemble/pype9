...
class AnonymousFunction(object):
    ...
    def __init__(self, nineml_params):
        ...
        expression_str.replace('&lt;', '<')
        expression_str.replace('&gt;', '>')
        for sym in self._math_constants + self._functions:
            expression_str.replace(sym, 'numpy.' + sym)
        # Create arguments list and replace any aliases
        args = []
        for name, alias in nineml_params.args.iteritems():
            if alias.strip():
                expression_str.replace(name, alias.strip())
        ...
        self.expression = eval('lambda {args}: {expr}'.format(args=args, expr=expression_str),
                               locals=consts)
