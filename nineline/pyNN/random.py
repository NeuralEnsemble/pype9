...
class RandomDistribution(pyNN.random.RandomDistribution):
    ...
    def _convert_params(cls, nineml_params):
        ...
        for name, p in nineml_params.iteritems():
            ...
            if p.unit == 'dimensionless':
                ...
                conv_param, param_units = convert_to_pyNN_units(
                    p.value, p.unit)
                if units is None:
                    units = param_units
                elif units != param_units:
                    raise Exception("Dimensions of random distribution "
                                    "parameters do not match ({} and {})"
                                    .format(units, param_units))
    ...
    def next(self, n=None, mask_local=None):
        """
        Wraps the PyNN RandomDistribution 'next' method and makes sure the
        units are attached
        """
...
class UniformDistribution(RandomDistribution):
    ...
    # needed because PyNN doesn't support kwargs for RandomDistribution
    # objects yet
