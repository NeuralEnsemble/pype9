...
class NetworkMLHandler(XMLHandler):
    ...
    Network = collections.namedtuple(
        'Network', 'id sim_params populations projections free_params')
    Population = collections.namedtuple('Population',
                                        ('id', 'celltype', 'morph_id', 'size',
                                         'structure', 'cell_params',
                                         'initial_conditions', 'flags',
                                         'not_flags'))
    Projection = collections.namedtuple('Projection',
                                        'id pre post connection weight delay '
                                        'synapse_family flags not_flags')
    Structure = collections.namedtuple('Structure', 'type somas args')
    StructureLayout = collections.namedtuple(
        'StructureLayout', 'pattern args distributions')
    CustomAttributes = collections.namedtuple(
        'CustomAttributes', 'constants distributions')
    Distribution = collections.namedtuple('Distribution',
                                          'attr type units seg_group '
                                          'component args')
    ...
    Destination = collections.namedtuple(
        'Destination', 'pop_id synapse segment')
    ...
    def startElement(self, tag_name, attrs):
        if self._opening(tag_name, attrs, 'network'):
            self.network = self.Network(
                attrs['id'], {}, [], [], self.FreeParameters({}, {}))
        elif self._opening(tag_name, attrs, 'freeParameters',
                           parents=['network']):
            pass
        elif self._opening(tag_name, attrs, 'flags',
                           parents=['freeParameters']):
            pass
        ...
        elif self._opening(tag_name, attrs, 'simulationDefaults'):
            pass
        elif self._opening(tag_name, attrs, 'temperature',
                           parents=['simulationDefaults']):
            self.network.sim_params['temperature'] = float(attrs['value'])
        elif self._opening(tag_name, attrs, 'timeStep',
                           parents=['simulationDefaults']):
            self.network.sim_params['timestep'] = float(attrs['value'])
        elif self._opening(tag_name, attrs, 'minDelay',
                           parents=['simulationDefaults']):
            self.network.sim_params['min_delay'] = float(attrs['value'])
        elif self._opening(tag_name, attrs, 'maxDelay',
                           parents=['simulationDefaults']):
            ...
            # of filter)
            self.pop_flags = filter(
                None, attrs.get('flags', '').replace(' ', '').split(','))
            self.pop_not_flags = filter(
                None, attrs.get('not_flags', '').replace(' ', '').split(','))
        elif self._opening(tag_name, attrs, 'structure',
                           parents=['population']):
            if self.pop_structure:
                raise Exception("The structure is specified twice in "
                                "population '{}'".format(self.pop_id))
            args = dict(attrs)
            self.pop_structure_type = args.pop('type')
            self.pop_structure_somas = None
            self.pop_structure_args = args
        elif self._opening(tag_name, attrs, 'somas',
                           parents=['population', 'structure']):
            args = dict(attrs)
            pattern = args.pop('pattern')
            self.pop_structure_somas = self.StructureLayout(pattern, args, [])
        elif self._opening(tag_name, attrs, 'cellParameters',
                           parents=['population']):
            pass
        elif self._opening(tag_name, attrs, 'initialConditions',
                           parents=['population']):
            pass
        elif self._opening(tag_name, attrs, 'const',
                           parents=['population', 'cellParameters']):
            self.pop_cell_params.constants[attrs['name']] = float(
                attrs['value'])  # FIXME: Units are ignored here
        elif self._opening(tag_name, attrs, 'const',
                           parents=['population', 'initialConditions']):
            self.pop_cell_params.constants[attrs['name']] = float(
                attrs['value'])  # FIXME: Units are ignored here
        elif (self._opening(tag_name, attrs, 'distribution',
                            parents=['population', 'cellParameters']) or
              self._opening(tag_name, attrs, 'distribution',
                            parents=['population', 'initialConditions']) or
              self._opening(tag_name, attrs, 'distribution',
                            parents=['population', 'structure', 'somas'])):
            args = dict(attrs)
            attribute = args.pop('attr')
            distr_type = args.pop('type')
            units = args.pop('units') if 'units' in args else None
            component = args.pop('component') if 'component' in args else None
            segmentGroup = args.pop(
                'segmentGroup') if 'segmentGroup' in args else None
            try:
                distr_param_keys = RANDOM_DISTR_PARAMS[distr_type]
            except KeyError:
                raise Exception("Unrecognised distribution type '{type}' used "
                                "to distribute cell attribute '{attribute}' "
                                "in population '{pop}'"
                                .format(type=distr_type, attribute=attribute,
                                        pop=self.id))
            try:
                distr_params = [float(args[arg]) for arg in distr_param_keys]
            except KeyError as e:
                raise Exception("Missing attribute '{distr_params}' for "
                                "'{type}' distribution used to distribute "
                                "cell attribute '{attribute}' in population "
                                "'{pop}'".format(distr_params=e,
                                                 type=distr_type,
                                                 attribute=attribute,
                                                 pop=self.id))
            distr = self.Distribution(attribute, distr_type, units,
            ...
            if self.proj_pre:
                raise Exception(
                    "The pre is specified twice in projection {}'"
                    .format(self.proj_id))
            self.proj_pre = self.Source(attrs['id'],
                                        attrs.get('terminal', None),
                                        attrs.get('segment', None))
        elif self._opening(tag_name, attrs, 'destination',
                           parents=['projection']):
            if self.proj_post:
                raise Exception("The destination is specified twice in "
                                "projection'{}'".format(self.proj_id))
            self.proj_post = self.Destination(attrs['id'],
                                              attrs.get('synapse', None),
                                              attrs.get('segment', None))
        elif self._opening(tag_name, attrs, 'connection',
                           parents=['projection']):
            if self.proj_connection:
                raise Exception("The connection is specified twice in "
                                "projection '{}'".format(self.proj_id))
            args = dict(attrs)
            pattern = args.pop('pattern')
...
#     """ Extracts the network specified in the given file and returns a
#     NetworkMLHandler with the parsed network specification.
#
#     @param filename: The location of the NINEML-Network file to read the
#     network from. """ parser = xml.sax.make_parser() handler =
#     NetworkMLHandler() parser.setContentHandler(handler)
#     parser.parse(os.path.normpath(filename)) return handler.network
