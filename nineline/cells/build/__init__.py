...
def get_build_paths(
        nineml_path, celltype_name, simulator_name, build_parent_dir=None):
    """
    return the respective source, install and optional compile directory paths
    for a given cell type

    @param nineml_path [str]: The path to the NCML file (used in the default
                              build directories)
    @param celltype_name [str]: The name of the cell type (used in the default
                                build directories)
    @param simulator_name [str] The name of the simulator, i.e. neuron or nest
                                (used in the default build directories)
    @param module_build_dir [str]: The path of the parent directory of the
                                   build directories (defaults to 'build' in
                                   the directory where the ncml path is
                                   located)
    ...
    src_dir = str(abspath(join(build_parent_dir, _SRC_DIR)))
    default_install_dir = str(abspath(join(build_parent_dir, _INSTALL_DIR)))
    compile_dir = str(abspath(join(build_parent_dir, _COMPILE_DIR)))
    params_dir = str(abspath(join(build_parent_dir, _PARAMS_DIR)))
...
def load_component_translations(biophysics_name, params_dir):
    """
    Loads component parameter translations from names to standard reference
    name (eg. 'e_rev', 'MaximalConductance') dictionary. For each file in the
    params directory with a '.py' extension starting with the celltype_name
    assume that it is a parameters file.

    @param biophysics_name [str]: The name of the cell biophysics to load the
                                  parameter names for
    ...
    for f_name in os.listdir(params_dir):
        if f_name.startswith(biophysics_name) and f_name.endswith('.py'):
            ...
            # Store in a dictionary of dictionaries indexed by component and
            # variable names
