...
def get_build_paths(
    ...
    @param nineml_path [str]: The path to the NCML file (used in the default build directories)
    ...
    if not build_parent_dir:
        build_parent_dir = os.path.normpath(os.path.abspath(os.path.join(os.path.dirname(nineml_path),
                                                                         _RELATIVE_BUILD_DIR,
                                                                         simulator_name, celltype_name)))
    src_dir = str(
        os.path.normpath(os.path.abspath(os.path.join(build_parent_dir, _SRC_DIR))))
    ...
    compile_dir = str(os.path.normpath(os.path.abspath(os.path.join(build_parent_dir,
                                                                    _COMPILE_DIR))))
    params_dir = str(
        os.path.normpath(os.path.abspath(os.path.join(build_parent_dir, _PARAMS_DIR))))
...
def load_component_translations(biophysics_name, params_dir):
    ...
    @param biophysics_name [str]: The name of the cell biophysics to load the parameter names for
    ...
    for f_name in os.listdir(params_dir):
        if f_name.startswith(biophysics_name) and f_name.endswith('.py'):
            ...
            loaded_props = run_path(
                os.path.join(params_dir, f_name))['properties']
