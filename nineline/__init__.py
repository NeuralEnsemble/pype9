...
def create_unit_conversions(basic_conversions, compound_conversions):
    ...
    for SI_unit, pyNN_unit in basic_conversions:
        ...
        # which may)
        basic_dict[Quantity(1, SI_unit).simplified._dimensionality] = Quantity(
            1, pyNN_unit)
    compound_dict = {}
...
class XMLHandler(xml.sax.handler.ContentHandler):
    ...
    def _opening(self, tag_name, attr, ref_name, parents=[],
        ...
        if tag_name == ref_name and self._parents_match(parents, self._open_components) and \
                all([(attr[key] == val or val is None) for key, val in required_attrs]):
    ...
    def _parents_match(self, required_parents, open_parents):
        if len(required_parents) > len(open_parents):
            return False
        for required, open_p in zip(reversed(required_parents), reversed(open_parents)):
