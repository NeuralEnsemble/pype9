...
class InsufficientTargetsWarning(Warning):
    pass
...
class BoltzmanWithDistance(object):
    ...
    def __call__(self, dist):
        values = (self.a1 - self.a2) / \
            (1 + np.exp((dist - self.x0) / self.dx)) + self.a2
        if self.min_value:
            values[values < self.min_value] = self.min_value
        return values
...
class MaskBased(object):
    ...
    def _probs_from_mask(self, mask):
        if self.prob:
            ...
            # If both self.prob and self.number are None (the default).
            if not self.number:
...
class CylinderMask(MaskBased):
    ...
    def __init__(self, radius, probability=None, axisX=0.0,
        ...
        if not norm:
            raise Exception(
                "Zero length vector provided as axis of CylinderMask.")
        self.axis /= norm
...
class EllipseMask(MaskBased):
    ...
    def __call__(self, d):
        mask = np.square(d[0] / self.x_scale) + \
            np.square(d[1] / self.y_scale) < 1
        return self._probs_from_mask(mask)
...
class EllipsoidMask(MaskBased):
    ...
    def __init__(
            self, x_scale, y_scale, z_scale, probability=None, number=None):
        """
        @param x: scale of the x axis of the ellipse
        @param y: scale of the y axis of the ellipse
        @param number: the mean number of connections to be generated. If None, all cells within the mask will be connected
...
class NewEllipsoidMask(MaskBased):
    ...
    def __init__(self, scale, orient_x, orient_y, orient_z,
                 isotropy, probability=None, number=None):
        """
    ...
    def __call__(self, displacement):
        ...
        working_matrix = axially_symmetric_tensor(
